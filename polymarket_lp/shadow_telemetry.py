...
def build_shadow_execution_telemetry(
    *,
    ...
    cancels = _cancels_from_quotes(
        quote, set(fills["client_order_id"]) if not fills.empty else set(), cfg
    )
    ...
    return {
        ...
        "summary": {
            ...
            "estimated_reward_usdc": float(rewards["estimated_reward_usdc"].sum())
            if not rewards.empty
            else 0.0,
            "paid_reward_usdc": float(rewards["paid_reward_usdc"].sum())
            if not rewards.empty
            else 0.0,
...
def _normalise_quotes(quotes: pd.DataFrame) -> pd.DataFrame:
    ...
    if "active_order_notional_pair" in out:
        out["active_order_notional_pair"] = pd.to_numeric(
            out["active_order_notional_pair"], errors="coerce"
        )
    return out.dropna(
...
def _orders_from_quotes(quotes: pd.DataFrame) -> pd.DataFrame:
    ...
    if "active_order_notional_pair" in quotes:
        orders["active_order_notional_pair"] = quotes[
...
def _fills_from_outcomes(
    ...
    outcomes["timestamp"] = pd.to_datetime(
        outcomes["timestamp"], utc=True, errors="coerce"
    )
    ...
    fills["fill_price"] = pd.to_numeric(
        fills["bid_price_quote"].fillna(fills["bid_price"]), errors="coerce"
    )
    fills["fill_size"] = pd.to_numeric(
        fills["size_shares_quote"].fillna(fills["size_shares"]), errors="coerce"
    )
...
def _cancels_from_quotes(
    ...
    ordered = quotes.copy()
    ordered["next_quote_ts"] = ordered.groupby(["condition_id", "side"])[
        "timestamp"
    ].shift(-1)
    cancels = ordered[
        ordered["next_quote_ts"].notna()
        & ~ordered["client_order_id"].isin(filled_order_ids)
    ].copy()
    if cancels.empty:
        return pd.DataFrame(columns=["client_order_id"])
    cancels["cancel_requested_ts"] = pd.to_datetime(
        cancels["next_quote_ts"], utc=True, errors="coerce"
    )
...
def _rewards_from_summary(
    summary: dict[str, Any], cfg: ShadowTelemetryConfig
) -> pd.DataFrame:
    estimated = float(summary.get("estimated_reward_accrual_usdc", 0.0))
    paid = (
        estimated * max(0.0, min(1.0, cfg.paid_reward_capture_rate))
