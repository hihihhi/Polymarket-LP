...
def evaluate_capital_risk_stress(
    ...
    *,
    ...
    for condition_id, group in grouped:
        side_cost = (
            group.assign(cost=group["bid_price"] * group["size_shares"])
            .groupby("side")["cost"]
            .sum()
        )
        ...
        market_rows.append(
            {
                ...
                "pair_cost_per_share": yes_bid + no_bid
                if paired_size > 0
                else math.nan,
    ...
    capped_by_market = float(
        market["one_side_worst_loss_to_zero"]
        .clip(upper=cfg.max_unpaired_per_market)
        .sum()
    )
    capped_by_cluster = float(by_cluster.clip(upper=cfg.max_cluster_unpaired).sum())
    capped_loss = min(
        unhedged_loss, cfg.max_total_unpaired, capped_by_market, capped_by_cluster
    )
    immediate_exit_loss = float(
        latest.drop_duplicates("condition_id")["size_shares"].sum() * cfg.exit_slippage
    )
    monthly_p05 = _captured_p05_monthly(target_status)
    daily_p05 = (
        monthly_p05 / cfg.days_per_month
        if monthly_p05 > 0 and cfg.days_per_month > 0
        else math.nan
    )
    unhedged_recovery_days = (
        unhedged_loss / daily_p05
        if daily_p05 and math.isfinite(daily_p05)
        else math.inf
    )
    capped_recovery_days = (
        capped_loss / daily_p05 if daily_p05 and math.isfinite(daily_p05) else math.inf
    )
    ...
    min_pair_edge = (
        float(market["locked_pair_edge_usdc"].min()) if len(market) else math.nan
    )
    max_pair_cost = (
        float(market["pair_cost_per_share"].max()) if len(market) else math.nan
    )
    ...
    metrics = {
        ...
        "max_single_market_active_fraction": max_single_market_active
        / cfg.initial_capital
        if cfg.initial_capital > 0
        else math.inf,
        "max_single_cluster_active_notional": max_single_cluster_active,
        "max_single_cluster_active_fraction": max_single_cluster_active
        / cfg.initial_capital
        if cfg.initial_capital > 0
        else math.inf,
        "single_market_worst_one_side_loss_to_zero": single_market_loss,
        "single_market_worst_one_side_loss_fraction": single_market_loss
        / cfg.initial_capital
        if cfg.initial_capital > 0
        else math.inf,
        "single_cluster_worst_one_side_loss_to_zero": max_single_cluster_loss,
        "single_cluster_worst_one_side_loss_fraction": max_single_cluster_loss
        / cfg.initial_capital
        if cfg.initial_capital > 0
        else math.inf,
        "all_active_unhedged_one_side_loss_to_zero": unhedged_loss,
        "unhedged_loss_fraction_of_capital": unhedged_loss / cfg.initial_capital
        if cfg.initial_capital > 0
        else math.inf,
        "configured_inventory_cap_loss_to_zero": capped_loss,
        "configured_inventory_cap_loss_fraction": capped_loss / cfg.initial_capital
        if cfg.initial_capital > 0
        else math.inf,
    ...
    capture_needed_gate_passed = (
        capture_needed_after_cap_loss <= cfg.max_capture_needed_after_cap_loss
    )
    gates = {
        ...
        "single_cluster_active_gate_passed": metrics[
            "max_single_cluster_active_fraction"
        ]
        <= cfg.max_single_cluster_active_fraction,
        "single_market_loss_gate_passed": metrics[
            "single_market_worst_one_side_loss_fraction"
        ]
        <= cfg.max_single_market_unhedged_loss_fraction,
        "single_cluster_loss_gate_passed": metrics[
            "single_cluster_worst_one_side_loss_fraction"
        ]
        <= cfg.max_single_cluster_unhedged_loss_fraction,
        "no_total_ruin_unhedged_gate_passed": unhedged_loss < cfg.initial_capital,
        "unhedged_loss_gate_passed": metrics["unhedged_loss_fraction_of_capital"]
        <= cfg.max_unhedged_loss_fraction,
        "configured_cap_loss_gate_passed": metrics[
            "configured_inventory_cap_loss_fraction"
        ]
        <= cfg.max_capped_loss_fraction,
        "configured_cap_recovery_gate_passed": capped_recovery_days
        <= cfg.max_capped_recovery_days,
        "capture_needed_after_cap_loss_gate_passed": capture_needed_gate_passed
        or not cfg.require_after_cap_target,
        "pair_lock_edge_gate_passed": min_pair_edge >= -1e-9
        and max_pair_cost <= 1.0 + 1e-9,
    }
    ...
    return {
        ...
        if gates["capital_risk_stress_passed"]
        else "capital_risk_stress_failed",
        "safety": _SAFETY,
    }
...
def _normalise_quotes(quotes: pd.DataFrame) -> pd.DataFrame:
    ...
    if "active_order_notional_pair" in q:
        q["active_order_notional_pair"] = pd.to_numeric(
            q["active_order_notional_pair"], errors="coerce"
        )
    else:
    ...
    if "cluster" not in q:
        q["cluster"] = "unknown"
    return q.dropna(
        subset=["timestamp", "condition_id", "bid_price", "size_shares"]
...
def _captured_p05_monthly(target_status: dict[str, Any] | None) -> float:
    ...
    if isinstance(stress, list):
        ...
        for row in stress:
            if (
                isinstance(row, dict)
                and abs(_float(row.get("capture_rate"), -1.0) - 0.5) < 1e-12
            ):
...
def _capture_needed(
    target_income: float, income_at_reference_capture: float, reference_capture: float
) -> float:
    if not (
