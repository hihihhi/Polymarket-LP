...
def load_snapshots(path: str | Path) -> pd.DataFrame:
    ...
    if "no_mid" not in df:
        df["no_mid"] = (
            ((df["no_best_bid"] + df["no_best_ask"]) / 2)
            if {"no_best_bid", "no_best_ask"}.issubset(df.columns)
            else 1 - df["yes_mid"]
        )
...
def filter_snapshots_for_strategy(
    snapshots: pd.DataFrame, cfg: LPConfig
) -> pd.DataFrame:
    ...
    if "reward_daily" in df:
        df = df[
            pd.to_numeric(df["reward_daily"], errors="coerce").fillna(0)
            >= cfg.min_reward_daily
        ]
    if "market_competitiveness" in df:
        df = df[
            pd.to_numeric(df["market_competitiveness"], errors="coerce").fillna(1)
            <= cfg.max_market_competitiveness
        ]
    if any(col in df.columns for col in ["category", "cluster", "tags"]):
        ...
        if excluded:
            df = df[
                ~token_text.apply(lambda text: bool(set(text.split(",")) & excluded))
            ]
...
def depth_capped_quote_size(
    row: pd.Series, cfg: LPConfig, *, base_size: float | None = None
) -> float:
...
def quote_one_sided_loss_to_zero(quote: dict[str, float | bool]) -> float:
    ...
    try:
        return max(float(quote["yes_bid"]), float(quote["no_bid"])) * float(
            quote["quote_size"]
        )
...
def quote_for_row(
    ...
    *,
    ...
    size = partial_rescue_residual_capped_quote_size(
        row, cfg, base_size=size, yes_bid=y_bid, no_bid=n_bid
    )
    ...
    eligible = (
        size > 0
        and size >= min_size
        and offset <= max_spread
        and pair_cost <= 1 - cfg.safety_margin + 1e-9
    )
    ...
    reward_density_per_day = (
        expected_reward_per_day / active_order_notional
        if active_order_notional > 0
        else 0.0
    )
    return {
        ...
        "quote_offset": offset,
        "quote_size": size,
        "active_order_notional": active_order_notional,
        "expected_reward_per_day": expected_reward_per_day,
        "reward_density_per_day": reward_density_per_day,
    }
...
def inv_notional(
    ...
    *,
    ...
    return sum(
        x.notional
        for x in inv
        if (condition_id is None or x.condition_id == condition_id)
        and (cluster is None or x.cluster == cluster)
    )
...
def handle_fill(
    *,
    ...
    cluster: str,
    cfg: LPConfig,
) -> tuple[list[InventoryLot], float, float, float, float, float]:
    rem = float(size)
    ...
    if rem > 1e-9:
        notional = price * rem
        can_open = (
            inv_notional(inventory, condition_id=condition_id) + notional
            <= cfg.max_unpaired_per_market
            and inv_notional(inventory) + notional <= cfg.max_total_unpaired
            and inv_notional(inventory, cluster=cluster) + notional
            <= cfg.max_cluster_unpaired
        )
        if can_open:
            inventory.append(
                InventoryLot(condition_id, side, price, rem, timestamp, cluster)
            )
            opened += rem
            events.append(
                {
...
def simulate_lp(
    ...
    df = (
        filter_snapshots_for_strategy(snapshots, cfg)
        .sort_values(["condition_id", "timestamp"])
    ...
    df["recent_vol"] = df.groupby("condition_id")["mid_change"].transform(
        lambda s: s.rolling(cfg.recent_vol_window, min_periods=1).mean()
    )
    df["recent_jump"] = df.groupby("condition_id")["mid_change"].transform(
    ...
    df["dt_seconds"] = (
        ...
        .dt.total_seconds()
        .clip(lower=0, upper=cfg.max_dt_seconds)
        .fillna(0)
    )
    inv: list[InventoryLot] = []
    events: list[dict[str, Any]] = []
    ...
    for ts, rows in df.groupby("timestamp", sort=True):
        ...
        for lot in inv:
            ...
            if (
                lot.price - mark >= cfg.exit_loss_cents
                or age >= cfg.max_unpaired_minutes
            ):
        ...
        if cfg.enable_rescue_quotes and inv:
            rows_by_condition = {
                str(row["condition_id"]): row for _, row in rows.iterrows()
            }
            for group in _inventory_rescue_groups(inv):
                ...
                current_same_side = [
                    lot
                    for lot in inv
                    if lot.condition_id == condition_id and lot.side == group["side"]
                ]
                    ...
                    events.append(
                        {
                            "timestamp": ts,
                            "condition_id": condition_id,
                            "event": "RESCUE_SKIPPED_CAPITAL",
                            "side": rescue["side"],
                            "bid_price": rescue["bid_price"],
                            "size": rescue["size"],
                ...
                events.append(
                    {
                        ...
                        "size": rescue["size"],
                        "worst_entry_price": worst_entry,
                        "max_rescue_bid": rescue["max_rescue_bid"],
                        "pair_cost": rescue["pair_cost"],
                        "pair_edge_per_share": rescue["pair_edge_per_share"],
                        "rescue_notional": rescue_notional,
                        "cluster": group["cluster"],
                    }
                )
                next_mid = (
                    row["next_no_mid"]
                    if rescue["side"] == "NO"
                    else row["next_yes_mid"]
                )
                if (
                    pd.notna(row["next_ts"])
                    and pd.notna(next_mid)
                    and float(next_mid) <= float(rescue["bid_price"])
                ):
                    inv, pp, ep, ps, os, cs = handle_fill(
                        inventory=inv,
                        events=events,
                        timestamp=pd.Timestamp(row["next_ts"]),
                        condition_id=condition_id,
                        side=str(rescue["side"]),
                        price=float(rescue["bid_price"]),
                        size=float(rescue["size"]),
        ...
        for _, row in rows.iterrows():
            if (
                float(row.get("recent_vol", 0.0)) > cfg.max_recent_vol
                or float(row.get("recent_jump", 0.0)) > cfg.max_recent_jump
            ):
                events.append(
                    {
                        "timestamp": ts,
                        "condition_id": row["condition_id"],
                        "event": "SKIP_VOLATILITY",
                        "recent_vol": float(row.get("recent_vol", 0.0)),
                        "recent_jump": float(row.get("recent_jump", 0.0)),
                        "cluster": row.get("cluster", "unknown"),
                    }
                )
                continue
            adaptive_offset = cfg.quote_offset + cfg.vol_quote_multiplier * float(
                row.get("recent_vol", 0.0)
            )
                ...
                events.append(
                    {
                        "timestamp": ts,
                        "condition_id": row["condition_id"],
                        "event": "SKIP_REWARD_DENSITY",
                        "reward_density_per_day": float(q["reward_density_per_day"]),
                        "cluster": row.get("cluster", "unknown"),
                    }
                )
                continue
            sort_key = float(
                q["reward_density_per_day"]
                if cfg.rank_by_reward_density
                else q["expected_reward_per_day"]
            )
        ...
        for lot in inv:
            pending_market_risk[lot.condition_id] = (
                pending_market_risk.get(lot.condition_id, 0.0) + lot.notional
            )
            pending_cluster_risk[lot.cluster] = (
                pending_cluster_risk.get(lot.cluster, 0.0) + lot.notional
            )
        for _, row, q in candidates:
            ...
            pending_market_risk[condition_id] = (
                pending_market_risk.get(condition_id, 0.0) + one_sided_risk
            )
            pending_cluster_risk[cluster] = (
                pending_cluster_risk.get(cluster, 0.0) + one_sided_risk
            )
            eligible += 1
            rw = (
                float(row["reward_daily"])
                * (float(row["dt_seconds"]) / 86400)
                * float(q["reward_share"])
            )
            reward += rw
            rew_ts += rw
            events.append(
                {
                    ...
                    "reward_usdc": rw,
                    "reward_share": q["reward_share"],
                    "our_score": q["our_score"],
                    "competitor_score": q["competitor_score"],
                    "active_order_notional": order_notional,
        ...
        eq.append(
            {
                "timestamp": ts,
                "reward_pnl_usdc": reward,
                "pair_pnl_usdc": pair_pnl,
                "exit_pnl_usdc": exit_pnl,
                "realized_pnl_usdc": realized,
                "mtm_inventory_pnl_usdc": mtm,
                "equity_realized": cfg.initial_capital + realized,
                "equity_mtm": cfg.initial_capital + realized + mtm,
                "open_inventory_lots": len(inv),
                "open_inventory_notional": inv_notional(inv),
                "active_order_notional": active,
                "reward_this_ts": rew_ts,
                "pair_pnl_this_ts": pair_ts,
                "exit_pnl_this_ts": exit_ts,
                "one_sided_open_shares": one_open,
                "paired_shares": paired_sh,
                "risk_cap_exit_shares": cap_sh,
    ...
    if not equity.empty:
        ...
        equity["drawdown_realized_usdc"] = (
            equity["equity_realized"] - equity["running_peak_realized"]
        )
        equity["drawdown_realized_pct"] = (
            equity["drawdown_realized_usdc"] / equity["running_peak_realized"]
        )
        equity["running_peak_mtm"] = equity["equity_mtm"].cummax()
        equity["drawdown_mtm_usdc"] = equity["equity_mtm"] - equity["running_peak_mtm"]
        equity["drawdown_mtm_pct"] = (
            equity["drawdown_mtm_usdc"] / equity["running_peak_mtm"]
        )
    return events_df, equity, summarize(events_df, equity, cfg)


def summarize(
    events: pd.DataFrame, equity: pd.DataFrame, cfg: LPConfig
) -> pd.DataFrame:
    if equity.empty:
        return pd.DataFrame()
    final = equity.iloc[-1]
    pnl_col = pd.to_numeric(
        events.get("pnl_usdc", pd.Series(dtype=float)), errors="coerce"
    ).fillna(0)
    losses = float(-pnl_col[pnl_col < 0].sum())
    gains = float(pnl_col[pnl_col > 0].sum())
    reward = (
        float(events.get("reward_usdc", pd.Series(dtype=float)).sum())
        if not events.empty
        else 0.0
    )
    pair = (
        float(
            events.loc[
                events.get("event", pd.Series(dtype=str)).eq("PAIR_MERGED"), "pnl_usdc"
            ].sum()
        )
        if not events.empty and "pnl_usdc" in events
        else 0.0
    )
    exitp = (
        float(
            events.loc[
                events.get("event", pd.Series(dtype=str)).isin(
                    ["EXIT_UNPAIRED", "RISK_CAP_IMMEDIATE_EXIT"]
                ),
                "pnl_usdc",
            ].sum()
        )
    ...
    one = (
        float(
            events.loc[
                events.get("event", pd.Series(dtype=str)).eq("OPEN_UNPAIRED"), "size"
            ].sum()
        )
        if not events.empty and "size" in events
        else 0.0
    )
    paired = (
        float(
            events.loc[
                events.get("event", pd.Series(dtype=str)).eq("PAIR_MERGED"), "size"
            ].sum()
        )
        if not events.empty and "size" in events
        else 0.0
    ...
    rescue_quotes = (
        int(events.get("event", pd.Series(dtype=str)).eq("RESCUE_QUOTE").sum())
        if not events.empty
        else 0
    )
    rescue_skipped = (
        int(
            events.get("event", pd.Series(dtype=str)).eq("RESCUE_SKIPPED_CAPITAL").sum()
        )
        if not events.empty
        else 0
    )
    daily = (
        equity.set_index("timestamp")["equity_mtm"]
        .resample("1D")
        .last()
        .dropna()
        .pct_change()
    ...
    return pd.DataFrame(
        [
            {
                **asdict(cfg),
                "total_reward_usdc": reward,
                "total_pair_spread_pnl_usdc": pair,
                "total_inventory_exit_pnl_usdc": exitp,
                "total_pnl_usdc": total,
                "return_on_initial_capital": total / cfg.initial_capital,
                "max_drawdown_realized_usdc": float(
                    equity["drawdown_realized_usdc"].min()
                ),
                "max_drawdown_realized_pct": float(
                    equity["drawdown_realized_pct"].min()
                ),
                "max_drawdown_mtm_usdc": float(equity["drawdown_mtm_usdc"].min()),
                "max_drawdown_mtm_pct": float(equity["drawdown_mtm_pct"].min()),
                "daily_sharpe_mtm": sharpe,
                "daily_sortino_mtm": sortino,
                "bad_day_p95_return": bad_day,
                "profit_factor_trading_only": gains / losses if losses else math.inf,
                "reward_to_trading_loss_ratio": reward / losses if losses else math.inf,
                "one_sided_open_shares": one,
                "paired_shares": paired,
                "pair_completion_ratio_shares": paired / max(paired + one, 1.0),
                "pair_completion_ratio_of_opened_shares": paired / one
                if one
                else math.inf,
                "rescue_quote_count": rescue_quotes,
                "rescue_skipped_capital_count": rescue_skipped,
                "max_open_inventory_notional": float(
                    equity["open_inventory_notional"].max()
                ),
                "avg_open_inventory_notional": avg_inv,
                "max_active_order_notional": float(
                    equity["active_order_notional"].max()
                ),
                "avg_active_order_notional": float(
                    equity["active_order_notional"].mean()
                ),
                "reward_per_dollar_avg_inventory": reward / avg_inv
                if avg_inv
                else math.inf,
                "pnl_per_dollar_avg_inventory": total / avg_inv
                if avg_inv
                else math.nan,
                "recovery_factor": total / max_dd if max_dd else math.inf,
                "quote_eligibility_intervals": int(equity["eligible_quotes"].sum()),
                "capital_skipped_intervals": int(equity["skipped_capital"].sum()),
            }
        ]
    )
...
def make_synthetic_snapshots(
    seed: int = 7, days: int = 14, n_markets: int = 30
) -> pd.DataFrame:
    ...
    for i in range(n_markets):
        ...
        for t in times:
            ...
            if rng.random() < (
                0.0009 if cat in {"politics", "culture", "economy"} else 0.003
            ):
            ...
            rows.append(
                {
                    "timestamp": t,
                    "condition_id": f"synthetic_{i:03d}",
                    "category": cat,
                    "cluster": cat,
                    "yes_mid": p,
                    "no_mid": 1 - p,
                    "reward_daily": rng.choice(
                        [25, 50, 100, 200, 500], p=[0.25, 0.3, 0.25, 0.15, 0.05]
                    ),
                    "max_incentive_spread": rng.choice([0.03, 0.04, 0.05]),
                    "min_incentive_size": rng.choice([10, 20, 25]),
                    "market_competitiveness": rng.beta(2, 4),
                }
            )
    return pd.DataFrame(rows)


def run_backtest_to_files(
    snapshots: pd.DataFrame, cfg: LPConfig, out_dir: str | Path
) -> pd.DataFrame:
    out = Path(out_dir)
