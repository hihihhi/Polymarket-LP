...
def collect_live_reward_snapshots(
    ...
    *,
    ...
    return frame.sort_values(
        ["reward_daily", "liquidity_clob"], ascending=[False, False]
...
def build_paper_quotes(snapshots: pd.DataFrame, cfg: LPConfig) -> pd.DataFrame:
    ...
    for ts, group in rows.groupby("timestamp", sort=True):
        ...
        for _, row, quote in candidates:
            ...
            active_market_risk[condition_id] = (
                active_market_risk.get(condition_id, 0.0) + one_sided_risk
            )
            active_cluster_risk[cluster] = (
                active_cluster_risk.get(cluster, 0.0) + one_sided_risk
            )
...
def analyze_paper_quotes(
    ...
    would_fill = (
        next_mid.notna() & bid.notna() & (next_mid <= bid - cfg.fill_mid_cross_buffer)
    )
    ...
    pair_fill_count = per_quote.groupby(["timestamp", "condition_id"])[
        "would_fill"
    ].transform("sum")
    per_quote["same_interval_pair_complete"] = per_quote[
        "would_fill"
    ] & pair_fill_count.ge(2)
...
def run_paper_analysis_to_files(
    *,
    ...
    (out / "paper_summary.json").write_text(
...
def add_risk_features(df: pd.DataFrame, cfg: LPConfig) -> pd.DataFrame:
    ...
    if rows.empty:
        return rows
    rows["mid_change"] = (
        rows.groupby("condition_id")["yes_mid"].diff().abs().fillna(0.0)
    )
...
def _snapshot_next_features(snapshots: pd.DataFrame) -> pd.DataFrame:
    ...
    if "yes_mid" not in snap:
        if {"yes_best_bid", "yes_best_ask"}.issubset(snap.columns):
            snap["yes_mid"] = (
                pd.to_numeric(snap["yes_best_bid"], errors="coerce")
                + pd.to_numeric(snap["yes_best_ask"], errors="coerce")
            ) / 2
        else:
            return pd.DataFrame()
    if "no_mid" not in snap:
        if {"no_best_bid", "no_best_ask"}.issubset(snap.columns):
            snap["no_mid"] = (
                pd.to_numeric(snap["no_best_bid"], errors="coerce")
                + pd.to_numeric(snap["no_best_ask"], errors="coerce")
            ) / 2
        else:
    ...
    snap = snap.sort_values(["condition_id", "timestamp"]).drop_duplicates(
        ["timestamp", "condition_id"], keep="last"
    )
...
def _normalise_quotes(quotes: pd.DataFrame) -> pd.DataFrame:
    quote = quotes.copy()
    if quote.empty or not {"timestamp", "condition_id", "side", "bid_price"}.issubset(
        quote.columns
    ):
        return pd.DataFrame()
    ...
    return quote.dropna(
        subset=["timestamp", "condition_id", "side", "bid_price"]
    ).reset_index(drop=True)
...
def _paper_summary(
    ...
    snap_ts = (
        pd.to_datetime(snapshots["timestamp"], utc=True, errors="coerce")
        if "timestamp" in snapshots
        else pd.Series(dtype="datetime64[ns, UTC]")
    )
    quote = _normalise_quotes(quotes)
    quote_pairs = (
        quote.drop_duplicates(["timestamp", "condition_id"])
        if not quote.empty
        else quote
    )
    pair_with_next = None
    ...
    reward_accrual = 0.0
    if pair_with_next is not None and {
        "reward_density_per_day",
        "active_order_notional_pair",
        "next_gap_seconds",
    }.issubset(pair_with_next.columns):
        gap = (
            pd.to_numeric(pair_with_next["next_gap_seconds"], errors="coerce")
            .clip(lower=0, upper=cfg.max_reward_gap_seconds)
            .fillna(0)
        )
        reward_per_day = pd.to_numeric(
            pair_with_next["reward_density_per_day"], errors="coerce"
        ).fillna(0) * pd.to_numeric(
            pair_with_next["active_order_notional_pair"], errors="coerce"
        ).fillna(0)
        reward_accrual = float((reward_per_day * gap / 86400).sum())
    quality_counts = _value_counts(
        snapshots.get("quote_data_quality", pd.Series(dtype=str))
    )
    cluster_loss = {}
    if not per_quote.empty and "cluster" in quote:
        tmp = per_quote.merge(
            quote[["timestamp", "condition_id", "side", "cluster"]],
            on=["timestamp", "condition_id", "side"],
            how="left",
        )
        pnl = pd.to_numeric(tmp["mark_to_next_pnl_if_filled"], errors="coerce").fillna(
            0
        )
        tmp = tmp.assign(mark_loss=(-pnl.clip(upper=0)))
        cluster_loss = {
            str(k): float(v)
            for k, v in tmp.groupby("cluster")["mark_loss"]
            .sum()
    ...
    stale_fill_rows = (
        int(per_quote["stale_fill"].sum()) if "stale_fill" in per_quote else 0
    )
    pair_complete_rows = (
        int(per_quote["same_interval_pair_complete"].sum())
        if "same_interval_pair_complete" in per_quote
        else 0
    )
    ...
    return {
        ...
        "unique_markets_quoted": int(quote["condition_id"].nunique())
        if not quote.empty
        else 0,
        ...
        "latest_active_pair_notional": float(active_by_ts.iloc[-1])
        if len(active_by_ts)
        else 0.0,
        "max_active_pair_notional": float(active_by_ts.max())
        if len(active_by_ts)
        else 0.0,
        "avg_active_pair_notional": float(active_by_ts.mean())
        if len(active_by_ts)
        else 0.0,
        ...
        "right_censored_pending_quote_rate": right_censored_pending_rows
        / max(len(per_quote), 1),
        "stale_quote_rate": float(per_quote["stale_quote"].mean())
        if "stale_quote" in per_quote and len(per_quote)
        else 0.0,
        "stale_fill_rows": stale_fill_rows,
        "stale_fill_rate": stale_fill_rows / max(fill_rows, 1),
        "estimated_mark_to_next_pnl_if_all_fills_usdc": float(
            per_quote["mark_to_next_pnl_if_filled"].sum()
        )
        if "mark_to_next_pnl_if_filled" in per_quote
        else 0.0,
...
def _value_counts(values: pd.Series) -> dict[str, int]:
    ...
    return {
        str(k): int(v)
        for k, v in values.fillna("unknown").astype(str).value_counts().items()
    }
    ...
    with _exclusive_output_lock(manifest_path.with_name(f"{manifest_path.name}.lock")):
        for i in range(iterations):
            ...
            manifest = {
                ...
                "active_pair_notional_latest_snapshot": _latest_active_pair_notional(
                    quotes
                ),
...
def _market_snapshot_row(
    *,
    ...
    if not isinstance(market, dict):
        return None
    reward_daily = sum(
        float(item.get("rewardsDailyRate") or 0.0)
    ...
    if reward_daily <= 0 or max_spread <= 0 or min_size <= 0:
        return None
    if not bool(market.get("acceptingOrders")) or not bool(
    ...
    if cfg.include_clob_books and len(token_ids) >= 2:
        ...
        no_bid, no_ask, no_bid_size, no_ask_size = _book_bid_ask(
            ...
            get_json=get_json,
            fallback_bid=float("nan"),
            fallback_ask=float("nan"),
        )
        quality = (
            "clob_book_both_sides"
            if pd.notna(no_bid) and pd.notna(no_ask)
            else "clob_book_yes_only"
        )

    yes_mid = _midpoint(
        yes_bid, yes_ask, _to_float(market.get("lastTradePrice"), default=float("nan"))
    )
    if pd.isna(yes_mid):
        return None
    ...
    return {
        ...
        "liquidity_clob": _to_float(
            market.get("liquidityClob")
            or market.get("liquidityNum")
            or market.get("liquidity")
        ),
        "volume24hr_clob": _to_float(
            market.get("volume24hrClob") or market.get("volume24hr")
        ),
        "spread": spread,
        "tags": tag_text,
...
def _parse_book_bid_ask(book: Any) -> tuple[float, float, float, float]:
    ...
    bid_size = (
        sum(size for price, size in bids if price == best_bid)
        if pd.notna(best_bid)
        else float("nan")
    )
    ask_size = (
        sum(size for price, size in asks if price == best_ask)
...
def _event_category(event: dict[str, Any]) -> tuple[str, str]:
    ...
    category = str(
        event.get("category") or (tags[0].get("slug") if tags else "") or "unknown"
    ).lower()
...
def _latest_active_pair_notional(quotes: pd.DataFrame) -> float:
    ...
    latest_quotes = quotes[
        pd.to_datetime(quotes["timestamp"], utc=True, errors="coerce").eq(latest)
    ]
    return float(
        latest_quotes.drop_duplicates("condition_id")[
            "active_order_notional_pair"
        ].sum()
    )
