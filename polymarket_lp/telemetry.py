...
def audit_execution_telemetry(
    *,
    ...
    unmatched_fills = int((~fills["client_order_id"].isin(order_ids)).sum()) if fill_count and order_ids else fill_count
    unmatched_fill_rate = unmatched_fills / max(fill_count, 1)

    cancel_latency = pd.Series(dtype=float)
    if not cancels.empty and {"cancel_requested_ts", "cancel_confirmed_ts"}.issubset(cancels.columns):
        cancel_latency = (cancels["cancel_confirmed_ts"] - cancels["cancel_requested_ts"]).dt.total_seconds().dropna()
    ...
    gates = {
        ...
        "cancel_telemetry_present": (not cfg.require_cancel_telemetry) or int(len(cancels)) >= cfg.min_cancels,
        "cancel_latency_passed": (not cfg.require_cancel_telemetry)
        or (not cancel_latency.empty and float(cancel_latency.max()) <= cfg.max_cancel_latency_seconds),
        "reward_payment_present": (not cfg.require_paid_rewards) or paid_reward >= cfg.min_paid_reward_usdc,
        "reward_capture_passed": (not cfg.require_paid_rewards)
        or (math.isfinite(reward_capture) and reward_capture >= cfg.min_reward_capture_rate),
    ...
    return {
        ...
        "metrics": {
            ...
            "max_cancel_latency_seconds": float(cancel_latency.max()) if not cancel_latency.empty else None,
            "median_cancel_latency_seconds": float(cancel_latency.median()) if not cancel_latency.empty else None,
        ...
        "gates": gates,
        "status": "deployment_telemetry_passed" if gates["deployment_telemetry_passed"] else "deployment_telemetry_incomplete",
...
def _normalise_fills(frame: pd.DataFrame | None) -> pd.DataFrame:
    ...
    if "fill_notional_usdc" not in out and {"fill_price", "fill_size"}.issubset(out.columns):
        out["fill_notional_usdc"] = pd.to_numeric(out["fill_price"], errors="coerce").fillna(0) * pd.to_numeric(
            out["fill_size"], errors="coerce"
        ).fillna(0)
