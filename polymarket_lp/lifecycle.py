...
CORE_STATES = ("book_snapshot", "ranking_decision", "quote_intent", "risk_gate", "sign", "submit")
...
def audit_order_lifecycle(events: pd.DataFrame, cfg: LifecycleAuditConfig | None = None) -> dict[str, Any]:
    ...
    for client_order_id, group in normalised.groupby("client_order_id", sort=True):
        per_order.append(_audit_one_order(str(client_order_id), group.sort_values("timestamp"), cfg))

    completed_orders = sum(1 for row in per_order if row["passed"])
    estimated_reward = _state_sum(normalised, "reward_estimate", "estimated_reward_usdc")
    paid_reward = _state_sum(normalised, "reward_paid", "paid_reward_usdc")
    reward_capture = paid_reward / estimated_reward if estimated_reward > 0 else math.inf if paid_reward > 0 else 0.0
    ...
    gate_checks = {
        ...
        "core_sequence_passed": all(row["gates"]["core_states_present"] for row in per_order),
        "acceptance_passed": all(row["gates"]["ack_or_reject_present"] for row in per_order),
        "queue_depth_passed": all(row["gates"]["queue_estimate_present"] for row in per_order),
        "time_order_passed": all(row["gates"]["time_order_passed"] for row in per_order),
        "submit_latency_passed": all(row["gates"]["submit_latency_passed"] for row in per_order),
        "ack_latency_passed": all(row["gates"]["ack_latency_passed"] for row in per_order),
        "cancel_latency_passed": all(row["gates"]["cancel_latency_passed"] for row in per_order),
        "fill_attribution_passed": all(row["gates"]["fill_attribution_passed"] for row in per_order),
        "rescue_decision_passed": all(row["gates"]["rescue_decision_passed"] for row in per_order),
        "final_pnl_passed": all(row["gates"]["final_pnl_present"] for row in per_order),
        "paid_reward_passed": (not cfg.require_paid_reward)
        or (paid_reward >= cfg.min_paid_reward_usdc and reward_capture >= cfg.min_reward_capture_rate),
    }
    ...
    return {
        ...
        "metrics": {
            ...
            "orders_with_fills": int(sum(1 for row in per_order if row["metrics"]["has_fill"])),
            "orders_with_rejects": int(sum(1 for row in per_order if row["metrics"]["has_reject"])),
            "max_cancel_latency_seconds": _max_metric(per_order, "cancel_latency_seconds"),
            "max_submit_latency_seconds": _max_metric(per_order, "sign_to_submit_seconds"),
            "max_ack_latency_seconds": _max_metric(per_order, "submit_to_ack_seconds"),
        },
        "gates": gate_checks,
        "blockers": _blockers(gate_checks, cfg),
        "per_order": per_order,
        "status": "deployment_lifecycle_passed" if gate_checks["deployment_lifecycle_passed"] else "deployment_lifecycle_incomplete",
...
def _audit_one_order(client_order_id: str, group: pd.DataFrame, cfg: LifecycleAuditConfig) -> dict[str, Any]:
    states = set(group["lifecycle_state"])
    has_fill = bool(states.intersection(FILL_STATES))
    has_reject = "reject" in states
    sequence_order = [STATE_ORDER.get(state, 10_000) for state in group["lifecycle_state"]]
    ...
    submit_to_ack = _state_delta_seconds(group, "submit", "ack") if "ack" in states else _state_delta_seconds(group, "submit", "reject")
    ...
    gates = {
        ...
        "queue_estimate_present": (not cfg.require_queue_estimate) or has_reject or "queue_estimate" in states,
        "time_order_passed": time_order_passed,
        "submit_latency_passed": sign_to_submit is not None and sign_to_submit <= cfg.max_sign_to_submit_seconds,
        "ack_latency_passed": submit_to_ack is not None and submit_to_ack <= cfg.max_submit_to_ack_seconds,
        "cancel_latency_passed": (not cfg.require_cancel_confirmation)
        or has_reject
        or (cancel_latency is not None and cancel_latency <= cfg.max_cancel_latency_seconds),
        "fill_attribution_passed": (not has_fill) or ("inventory_update" in states and "fee_slippage" in states),
        "rescue_decision_passed": (not cfg.require_rescue_decision_for_fills)
        or (not has_fill)
        or bool(states.intersection(RESCUE_STATES) and (fill_to_rescue is None or fill_to_rescue <= cfg.max_fill_to_rescue_seconds)),
        "final_pnl_present": (not cfg.require_final_pnl) or _state_has_numeric(group, "final_pnl_attribution", "final_pnl_usdc"),
    }
    ...
    return {
        ...
        "metrics": {
            ...
            "estimated_reward_usdc": _state_sum(group, "reward_estimate", "estimated_reward_usdc"),
            "paid_reward_usdc": _state_sum(group, "reward_paid", "paid_reward_usdc"),
            "final_pnl_usdc": _state_sum(group, "final_pnl_attribution", "final_pnl_usdc"),
        },
    }
...
def _normalise_events(events: pd.DataFrame | None) -> tuple[pd.DataFrame, list[str]]:
    if events is None or events.empty:
        return pd.DataFrame(columns=["timestamp", "client_order_id", "lifecycle_state"]), []
    out = events.copy()
...
def _state_delta_seconds(group: pd.DataFrame, start_state: str, end_state: str) -> float | None:
...
def _fill_to_rescue_seconds(group: pd.DataFrame) -> float | None:
    fill_ts = min((_first_state_ts(group, state) for state in FILL_STATES if _first_state_ts(group, state) is not None), default=None)
    rescue_ts = min((_first_state_ts(group, state) for state in RESCUE_STATES if _first_state_ts(group, state) is not None), default=None)
...
def _state_has_numeric(group: pd.DataFrame, state: str, column: str) -> bool:
    rows = group[group["lifecycle_state"].eq(state)]
    return bool(not rows.empty and column in rows and pd.to_numeric(rows[column], errors="coerce").notna().any())
...
def _empty_result(cfg: LifecycleAuditConfig, raw_rows: int, missing_required: list[str]) -> dict[str, Any]:
    gates = {
    ...
    return {
        ...
        "gates": gates,
        "blockers": _blockers(gates, cfg) + ([f"missing required columns: {missing_required}"] if missing_required else []),
        "per_order": [],
